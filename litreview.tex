...
Memcached is one of the first distributed key-value stores. Unlike Dynamo and Voldemort, the distributed nature of Memcahced does not provide for high availability. Instead, the distributed nature allows Memcached to scale to many machines as a means of accessing more memory to store values in. Memcached itself does not provide any replication or cross server communication. Dynamo and Voldemort provide high availability, replication, and throughput at the cost of consistency. Memcached is very similar to a distributed hash table with multi-level lookups where Dynamo and Voldemort provide more robust cosistent hashing based solutions for storing, querying, and modifying data. Voldemort provides several extensions on top of Dynamo including build-in compression and (de)-serialization.
...
\begin{table}
    \begin{adjustbox}{center} 
        \begin{tabulary}{1.3\linewidth}{|c|L|L|L|}
            ...
            \hline 
            \textbf{Architecture} & 2-way hash table & Ring of nodes with coordinator / preference lists & Ring of nodes with coordinator / preference lists \\ 
            \hline 
