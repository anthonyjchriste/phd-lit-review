...
Some of the above mentioned service models are of particular interest for a survey examining cloud computing and sensor networks. We will examine these in more detail in sections \ref{sec:saas}.

\subsubsection{Sensing as a Service} \label{sec:saas}
...
Rao et al.\cite{rao_cloud_2012} mention several research challenges for SaaS including the need for a standard distributed computing framework for distributed big data as well as a framework for the real-time monitoring of events. Perera et al\cite{perera_sensing_2014} introduce the idea of sensing as a service being an ideal model for smart cities which are supported by IoT sensors.

Finally, the \textit{sensor data consumers} layer consist of data consumer who must register with the ESPs and provide valid digital certificates. Consumers can either deal with SPs directly or deal with ESPs. The benefit to dealing with SPs is reduced cost of communications with the ESP. The benefit of dealing with ESPs is higher level querying to data and the ability to query data across multiple SPs.
